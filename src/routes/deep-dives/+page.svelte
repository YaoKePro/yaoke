...
<div class="max-w-6xl mx-auto px-6 py-16">
    ...
    {#each series as s}
      <div class="series-card group">
        <div class="flex flex-col lg:flex-row gap-8">
          ...
          <div class="flex-1">
            <div class="mb-6">
              <h2 class="text-3xl font-serif font-bold text-primary mb-3 group-hover:text-primary-hover transition-colors">
                {s.name}
              </h2>
            ...
            <div class="chapters-preview mb-6">
              <h3 class="text-sm font-semibold text-tertiary uppercase tracking-wide mb-3">
                Chapters
              </h3>
              <div class="space-y-2">
                {#each s.chapters as chapter}
                  <a href={`/blogs/${chapter.slug}`} class="chapter-link">
                    <span class="chapter-number">Chapter {chapter.metadata.chapter}</span>
                    <span class="chapter-title">
                      {chapter.metadata.title.split(':')[1]?.trim() || chapter.metadata.title}
                    </span>
  ...
  {#if series.length === 0}
    <div class="text-center py-16">
      <p class="text-tertiary">No deep dives available yet. Check back soon!</p>
    </div>
  {/if}
